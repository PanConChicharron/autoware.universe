 ...
 *     --simulator-yaml $(ros2 pkg prefix j6_gen2_description)/share/j6_gen2_description/config/simulator_model.param.yaml \
 *     --vehicle-info-yaml $(ros2 pkg prefix j6_gen2_description)/share/j6_gen2_description/config/vehicle_info.param.yaml \
  ...
  const float goal_x, const float speed, const float ds, const int sim_steps)
{
  ...
  const std::size_t point_count =
    static_cast<std::size_t>(std::ceil(static_cast<double>(path_length) / static_cast<double>(ds))) +
    1U;
...
TrajectoryPoint makeStraightPathPointAtArcLength(const float s, const float speed, const float s_max)
{
